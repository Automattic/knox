...
function startStream(outputStream, req, fn) {
  var contentLength = req.getHeader('content-length');

  registerReqListeners(req, fn);
  outputStream.on('error', fn);
...
Client.prototype.putFile = function(src, filename, headers, fn){
  ...
  // avoid setting header 'expect' to fire the request
  var req = self.request('PUT', filename, headers);
  ...
  fs.stat(src, function (err, stat) {
    ...
    Object.keys(newHeaders).forEach(function(key) {
      req.setHeader(key, newHeaders[key]);
    });

    var stream = fs.createReadStream(src);
    startStream(stream, req, fn);
  });
