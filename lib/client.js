...
var Client = module.exports = exports = function Client(options) {
  ...
  // See https://github.com/aws/aws-sdk-js/blob/master/lib/services/s3.js#L38 
  if (this.secure && /\./.test(options.bucket)) {
    this.endpoint = join(domain + port, options.bucket);
  } else {
...
Client.prototype.request = function(method, filename, headers){
  var urlString = this.url(ensureLeadingSlash(filename))
    , parsedUrl = url.parse(urlString)
    , options = { hostname: parsedUrl.hostname, agent: this.agent }
    , date = new Date
    , headers = headers || {};
  ...
  options.path = encodeSpecialCharacters(parsedUrl.path);
