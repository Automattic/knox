...
/**
 * PUT the file at `url` to `filename`, with callback `fn`
 * receiving a possible exception, and the response object.
 *
 * Example:
 *
 *    client
 *     .putUrl('http://domain.com/image.jpg', '/test/image.jpg', function(err, res){
 *       if (err) throw err;
 *       console.log(res.statusCode);
 *       console.log(res.headers);
 *     });
...
Client.prototype.putUrl = function(url, filename, headers, fn){
  var self = this;
