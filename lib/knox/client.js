...
ChunkBuilder.from_stream = function(stream, callback){
	var obj = new this();
	stream.on('data', function(chunk) { this.data += chunk }.bind(this));
	stream.on('error', function(e) { this.error = e; this.completed = true }.bind(this));
...
Client.prototype.getAllParts = function(filename, uploadInfo, headers, fn){
	if(typeof headers == 'function') {fn = headers; headers = {}; }
	function processPartList(err, pinfo){
		...
		else this.client.getParts(filename, uploadId, headers, processPartList.bind(this));
	}
	this.getParts(filename, uploadInfo, headers, processPartList.bind({'parts': [], 'client': this}));
};
