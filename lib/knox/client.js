 ...
 * @function
 */
ChunkBuilder.from_response = function(stream, callback){
	var obj = new this();
	obj.complete = function() { this.completed = true; callback(this.error, this.data) }.bind(obj);
	stream.on('data', obj.append_chunk);
	stream.on('error', obj.catch_error);
	stream.on('end', obj.complete);
	return obj;
};
...
function ListPartsResult(rawxml){
	...
	if(typeof rawxml == 'string'){
		...
		var parts = get_element_content(rawxml, 'Part', true);
		for(var i in parts)
			parts.push(new PartInfo(parts[i]));
	}
}
...
Client.prototype.beginUpload = function(filename, headers, fn){
	...
	req.on('response', function(response){
		...
		else ChunkBuilder.from_response(response, function(err, data) { fn(err, new UploadInitResult(data)) });
	});
	req.end();
};
 ...
 * @api public
 */
Client.prototype.listParts = function(filename, parameters, headers, fn){
	...
	req.on('response', function(response){
		response.setEncoding('utf8');	//TODO: Encoding may be detected in more flexible way (or detected automatically).
		if(response.statusCode != 200) fn({'statusCode': response.statusCode}, null);
		else ChunkBuilder.from_response(response, function(err, data) { fn(err, new ListPartsResult(data)) });
	});
	req.end();
};
...
Client.prototype.completeUpload = function(filename, uploadId, parts, headers, fn){
	...
	var req = this.request('POST', filename, headers);
	req.on('response', function(response){
		var responseHandler = {'error': null, 'data': ''};
		response.setEncoding('utf8');
		response.on('error', function(e){ this.error = e }.bind(responseHandler));
		response.on('data', function(chunk) { this.data += chunk }.bind(responseHandler));
		response.on('end', function(){ 
			if(this.error) fn(this.error, null);
			else if(response.statusCode == 200) fn(null, this.data);
			else fn({'status': response.statusCode}, this.data);
...
Client.prototype.putPart = function(filename, partNumber, uploadId, buf, fn){
	...
	if(fn){
		...
		'Expect': '100-continue'});
		req.on('response', function(response){
			var responseHandler = {error: null};
			response.on('error', function(e) { this.error = e }.bind(responseHandler));
			response.on('end', function() {
						if(this.error) fn(this.error, null);
