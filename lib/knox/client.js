 ...
 * @api public
 */
Client.prototype.list_parts = function(filename, parameters, headers, fn){
 ...
 * @param {Object} headers Additional request headers.
 * @param {Function} fn Callback function that accepts true if multipart upload is aborted successfully; otherwise, false.
 * @api public
 */
Client.prototype.abort_upload = function(filename, uploadId, headers, fn){
	//Validate function arguments
	if(typeof headers == 'function') { fn = headers; headers = {}; }
	filename += '&uploadId=' + uploadId;
	//Send multipart upload abortion.
	var req = this.request('DELETE', filename, headers);
	req.on('response', function(response){ fn(response.statusCode == 200) });
	req.end();
};
