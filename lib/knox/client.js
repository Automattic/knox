...
/**
 * Copy a given source file from a given src ('/bucket/url_to_file')
 * To dest path 
 *   	Example:
 *   	// Copies file from /src_bucket/file to current bucket's /file 
 *   	var r = req.copyObject('/src_bucket/file', '/file');
 ...
 *   	r.end(); // Must do!
 */
Client.prototype.copyObject = function(src, dest, headers){
	headers = utils.merge({
			  'Content-Length': 0
			, 'x-amz-copy-source': src 
			}, headers || {});

	return this.request('PUT', dest, headers);
};
