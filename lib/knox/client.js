...
Client.prototype.putAcl= function(acl, filename, headers, fn) {
  var self = this;
  if ('function' == typeof headers) {
    fn = headers;
    headers = {};
   ...
   headers = utils.merge({
        'Content-Length': acl.length
    }, headers);

    self.request('PUT', filename+'?acl', headers).on('response', function(res){
        res.on('data', function(data) {
            console.log(data.toString('utf-8'));
          });
      fn(null, res);
...
Client.prototype.putStream = function(stream, filename, headers, fn){
  var req;
  stream.proxyReady = false;
  stream.proxyBuffer = [];

  stream
  .on('error', function(err){fn(err); })
  .on('data', function(chunk){
      if(stream.proxyReady) {
        req.write(chunk);
        hash.update(chunk);
        uploadSize += chunk.length;
      }
      else {
        stream.proxyBuffer.push(chunk);
        uploadSize += chunk.length;
        hash.update(chunk);
      }
    })
  ...
  fs.stat(stream.path || '.', function(err, stat){
    ...
    if(!headers['Content-Length']) {
      headers['Content-Length'] = stream.path ? stat.size : 0;
    }
    if(!headers['Content-Type']) {
      headers['Content-Type'] = stream.path ? mime.lookup(stream.path) : 'x-application';
    }
    req = self.put(filename, headers);
    stream.proxyReady = true;
    if(stream.proxyBuffer) {
      for(var index = 0; index < stream.proxyBuffer.length; index++) {
        req.write(stream.proxyBuffer[index]);
      }
    }
    ...
    req.on('response', function(res){
        fn(null, res, hash.digest('hex'), uploadSize);
      });
  });
 };
